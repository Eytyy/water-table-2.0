...
class PopulationVisulization extends Component {
	...
	getCurrentPopulation = ()  => {
		let index = this.population_data.findIndex(({ year }) => year === this.next_year_population);
	...
	setupPopulationCircles = () => {
		const max = this.getNodesRangeCount(population[population.length - 1].population);
		for (let i = 0; i < max; i++) {
	...
	updateActiveYear = (payload) => {
		...
		let yearIndex = population.findIndex(({ year }) =>  year === parseInt(payload, 10));
		...
		this.next_radius = this.rendered === 0 ? this.radiusRangeSize(yearIndex) : this.reverse ? this.radiusRangeSize(yearIndex - 1) : this.radiusRangeSize(yearIndex + 1);
	...
	componentDidMount() {
		...
		this.next_year_population = this.getNodesRangeCount(population[0].population);
		this.drawPopulation();
	}
	...
	getPopulation = () => {
		return format(",.0f")(this.population_data.find(({ year }) => year === parseInt(this.state.activeYear, 10)).population).replace(/,/g, ' ');
	}
	
	
	render() {
		return <div id="population__visualization" className="visualization-wrapper">
			<canvas
			...
			<div className="visualization__content">
				...
				<div className="visualization__content__data">{this.getPopulation()}</div>
			</div>
		</div>
	}
}
