...
const DesalinationSvg = ({ config, active, onClick, activeLayer }) => {
	return (
		<svg width="1080" height="1580" viewBox="0 0 1080 1580" className={`${activeLayer === 'desalination' ? 'is-active' : 'is-hidden'} resources resources--desalination`}>
			<g stroke="none" strokeWidth="1" fill="none" fillRule="evenodd" transform="translate(180, 190)">
				{
					config.entries.map(({ id, position }) => 
						<g
							onClick={e => onClick(e, 'desalination')}
							className="desalination"
							key={id}
							id={id}
...
export default DesalinationSvg;