...
const GroundWaterSVG = ({ config, activeGroundWater, onGroundWaterClick, activeLayer }) => {
	return (
		<svg width="1080" height="1580" viewBox="0 0 1080 1580" className={`${activeLayer === 'ground' ? 'is-active' : 'is-hidden'} resources resources--ground`}>
			<g stroke="none" strokeWidth="1" fill="none" fillRule="evenodd" transform="translate(180, 190)">
				{
					config.entries.map(({ id, position }) => 
						<g
							onClick={onGroundWaterClick} className="ground"
							key={id}
							id={id}
							transform={`translate(${position.x}, ${position.y}) ${activeGroundWater !== id ?  'scale(1, 1)' : 'scale(3, 3)'}`}>
