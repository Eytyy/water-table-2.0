...
const SupplySVG = ({ config, active, onClick, activeLayer }) => {
	return (
		<svg width="1080" height="1580" viewBox="0 0 1080 1580" className={`${activeLayer === 'supply' ? 'is-active' : 'is-hidden'} resources resources--supply`}>
			<g stroke="none" strokeWidth="1" fill="none" fillRule="evenodd" transform="translate(180, 190)">
				{
					config.entries.map(({ id, position }) => 
						<g
							onClick={(e) => onClick(e, 'supply')}
							className="supply"
							key={id}
							id={id}
